...
def process_file(input_file):
    ...
    with open(input_file, 'r') as file:
        for line in file:
            # Supposons que le fichier ait un format spécifique : domain, subdomain, http_codes, ports, ip_address, scan_time
            data = line.strip().split(',')
            if len(data) == 6:
                domain, subdomain, http_codes, ports, ip_address, scan_time = data
                insert_scan_result(scan_id, domain, subdomain, http_codes, ports, ip_address, scan_time)
            else:
